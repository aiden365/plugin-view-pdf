...
在现有 “XCode Viewer” 工具窗基础上增加“伪装模式/查看 PDF”能力：工具窗左右分栏，左侧显示当前项目目录树；右侧默认显示所选 Java 文件内容（仅 `.java` 点击有效，用于掩饰）；在标题栏按钮切换为 “查看PDF/取消伪装” 后，右侧切换为 PDF 预览，并将右侧内容区背景设置为 `RGB(43,45,48)`。同时保持原有夜间模式渲染与全局滚动快捷键，并按约定：快捷键仅在 PDF 模式生效。
